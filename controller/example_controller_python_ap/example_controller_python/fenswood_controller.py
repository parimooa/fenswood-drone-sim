    ...
    def timer_callback(self):
        ...
        if self.controller_command == 'Run':
            ...
            if self.state == 'Takeoff':
                ...
                self.logger.info(
                    f"Takeoff position: {self.vehicle_position.pose.position.z}:{self.flight_height * 0.95}")
            ...
            if self.state == 'Flight':
                ...
                duration_since_start = (
                            current_time - self.flight_start_time) if self.flight_start_time is not None else None
                if self.flight_start_time is None:
                    self.flight_start_time = current_time
                    self.flight_previous_time = current_time
                    self.previous_position = (
                    0, self.vehicle_position.pose.position.x, self.vehicle_position.pose.position.y, 0)
                elif duration_since_start > self.flight_duration:
                    self.logger.info("Flight Completed, Going for Landing")
                    self.state = 'Land'
                elif duration_since_start > rclpy.duration.Duration(
                        seconds=self.trajectory_points[self.trajectory_idx][0]):
                    ...
                    time_diff = duration_since_start.nanoseconds - rclpy.duration.Duration(
                        seconds=self.previous_position[0]).nanoseconds
                    traj_point = self.trajectory_points[self.trajectory_idx]  # (time, x, y, yaw)
                    full_duration = rclpy.duration.Duration(
                        seconds=traj_point[0] - self.previous_position[0]).nanoseconds
